...
**HackKit Web App**:
A fully built HackKit application that assembles Better Auth, the HackKit Better Auth plugin, UI, and routes into a working hackathon site. Server code calls inferred `auth.api` endpoints and browser code uses the HackKit client plugin — see [`docs/guides/next-integration.md`](docs/guides/next-integration.md).
_Avoid_: Core, kernel
...
**HackKit CLI**:
A command-line tool for creating HackKit projects and synchronizing plugin-provided project files. Better Auth generates the unified auth and HackKit schema; the application owns migrations and seed data.
_Avoid_: Runtime, core

**HackKit Plugin**:
An extension installed inside the HackKit Better Auth plugin that may add capabilities, data models, permissions, explicit endpoints, hooks, and project files.
...
**Better Auth Database Context**:
The database adapter and schema lifecycle owned by Better Auth. The HackKit Better Auth plugin contributes models to this context and uses it privately for persistence.
_Avoid_: HackKit database configuration, parallel database runtime, HackKit-owned migrations
...
-   **HackKit CLI** runs from a **HackKit Web App** project directory (not a monorepo root) and reads that app’s `hackkit.config.ts` for plugins and project files.
...
-   Better Auth's CLI generates one unified schema from Better Auth and installed HackKit plugin models.
-   The **HackKit Web App** owns migration generation, application, and seed data.
...
-   **HackKit Plugins** contribute models and explicit endpoints through the **HackKit Better Auth Plugin**.
...
-   The **HackKit Better Auth Plugin** converts Core and nested-plugin models into Better Auth plugin schema contributions.
-   Better Auth's adapter translates endpoint persistence operations for the application's configured database.
...
-   The **HackKit Web App** commits one Better Auth-generated schema containing auth, HackKit, and installed extension tables, then delegates migrations entirely to its chosen migration tool.
